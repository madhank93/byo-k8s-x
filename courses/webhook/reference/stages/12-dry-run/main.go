...
func validate(w http.ResponseWriter, r *http.Request, cs kubernetes.Interface, ns string) {
	...
	// The decision is made above and the side effect happens here, so honouring
	// a dry run is one branch around the second half. The verdict a hypothetical
	// request gets is provably the one a real request would have got.
	if !dryRun(review.Request) {
		if err := recordVerdict(r.Context(), cs, ns, name, allowed); err != nil {
