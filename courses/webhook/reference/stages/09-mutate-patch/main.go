...
func mutate(w http.ResponseWriter, r *http.Request) {
	...
	// patch triggers a second call — so applying this twice has to change
	// nothing the first pass did not already change.
	if pod.Annotations[injectedAnnotation] == "true" {
		respond(w, uid, true, "")
		return
	}

	// `add` cannot create the map it writes into: on a pod with no annotations
	// at all, patching the key without creating the parent first fails.
