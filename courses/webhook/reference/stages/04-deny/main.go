...
func run() error {
	...
	if err := register(ctx, cs, url, caPEM); err != nil {
		return err
	}
	defer func() {
...
func validate(w http.ResponseWriter, r *http.Request) {
	...
	if err := json.Unmarshal(review.Request.Object.Raw, &pod); err != nil {
		http.Error(w, fmt.Sprintf("decode pod: %v", err), http.StatusBadRequest)
		return
	}
	...
	if pod.Labels["owner"] == "" {
		...
		status = &metav1.Status{
			...
			Message: fmt.Sprintf("pod %q has no owner label: every pod must say who owns it", name),
		}
	}
...
// clientConfig loads the kubeconfig the way every Kubernetes tool does.
...
func register(ctx context.Context, cs kubernetes.Interface, url string, caBundle []byte) error {
