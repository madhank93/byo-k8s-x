...
One program that grows for twenty-six stages: an HTTPS server that registers
...
3. **Stages 12-26 — behaving in production.** Honour a dry run, choose a
   ...
   tell an eviction from a delete, judge the delete itself, guard every way
   into a running pod, let a namespace go, move a rule's data out of code,
   and hear about every version of what you judge.
