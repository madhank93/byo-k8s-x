...
//
// It lists first and watches from the list's resourceVersion, because those
// two calls are one operation: the list is the state at that version, and the
// watch is every change after it. Watching without listing misses what was
// already there; listing and then watching from "now" loses whatever changed
// in between.
func observe(ctx context.Context, dyn dynamic.Interface, ns string) error {
	...
	for ctx.Err() == nil {
		...
		if err != nil {
			return fmt.Errorf("watch websites: %w", err)
		}
		rv, err = drain(ctx, w, rv)
		if err != nil {
			return err
		}
		if rv == "" {
			// The server no longer has history back to where we were, so the
			// only safe move is to ask for the state again.
			list, err := client.List(ctx, metav1.ListOptions{})
			if err != nil {
				return fmt.Errorf("relist websites: %w", err)
			}
			rv = list.GetResourceVersion()
		}
	}
...
// drain prints one watch's events and returns the resourceVersion to resume
// from. An empty resourceVersion means the watch expired and the caller has to
// list again.
func drain(ctx context.Context, w watch.Interface, rv string) (string, error) {
	defer w.Stop()
	for {
		select {
		case <-ctx.Done():
		...
		case ev, open := <-w.ResultChan():
			if !open {
				// A closed channel is routine — watches are cut off by
				// timeouts and restarts — so resume rather than exit.
				return rv, nil
			}
