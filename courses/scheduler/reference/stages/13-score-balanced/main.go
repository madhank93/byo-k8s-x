...
func run() error {
	...
	pick := func(pod *corev1.Pod) string {
		...
		// left is one answer and evenness is another, and a node has to do
		// well at both: the scores are added, the way a real scheduler sums
		// the plugins that scored a node.
		var best []string
		bestScore := int64(-1)
		for _, n := range candidates {
			switch s := leastAllocated(pod, n, placed) + balancedAllocation(pod, n, placed); {
			case s > bestScore:
