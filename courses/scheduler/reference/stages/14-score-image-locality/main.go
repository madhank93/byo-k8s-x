...
func run() error {
	...
	pick := func(pod *corev1.Pod) string {
		...
		for _, n := range all {
			if isReady(n) && schedulable(pod, n) && tolerates(pod, n) && selector.Matches(labels.Set(n.Labels)) && affinityMatches(pod, n) && fits(pod, n, placed) {
		...
		// left is one answer, evenness is another, and how soon the pod can
		// start is a third: the scores are added, the way a real scheduler
		// sums the plugins that scored a node.
		var best []string
		bestScore := int64(-1)
		for _, n := range candidates {
			switch s := leastAllocated(pod, n, placed) + balancedAllocation(pod, n, placed) + imageLocality(pod, n); {
			case s > bestScore:
