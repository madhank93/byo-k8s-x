...
func run() error {
	...
	pick := func(pod *corev1.Pod) string {
		...
		for _, n := range all {
			if isReady(n) && schedulable(pod, n) && tolerates(pod, n) && selector.Matches(labels.Set(n.Labels)) && affinityMatches(pod, n) && topologySatisfied(pod, n, all, placed) && podAffinitySatisfied(pod, n, all, placed) && volumesReachable(pod, n, claimLister, pvLister) && fits(pod, n, placed) {
