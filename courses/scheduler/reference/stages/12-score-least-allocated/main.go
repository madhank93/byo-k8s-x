...
func run() error {
	...
	pick := func(pod *corev1.Pod) string {
		...
		// Fitting is a yes or no; it does not say which yes is better. The
		// node with the most room left after this pod lands is the answer.
		var best []string
		bestScore := int64(-1)
		for _, n := range candidates {
			switch s := leastAllocated(pod, n, placed); {
			case s > bestScore:
