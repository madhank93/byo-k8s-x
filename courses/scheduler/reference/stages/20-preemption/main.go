...
// filter but the one about space. Preemption asks the same question, since a
// node this pod may not run on is not made suitable by evicting anyone.
func feasible(pod *corev1.Pod, node *corev1.Node, all []*corev1.Node, placed []*corev1.Pod, claims corelisters.PersistentVolumeClaimLister, pvs corelisters.PersistentVolumeLister) bool {
	// A pod's node selector names labels a node must carry, every one of them;
	// an empty selector matches every node.
	selector := labels.SelectorFromSet(pod.Spec.NodeSelector)
	return isReady(node) && schedulable(pod, node) && tolerates(pod, node) &&
		selector.Matches(labels.Set(node.Labels)) && affinityMatches(pod, node) &&
		topologySatisfied(pod, node, all, placed) && podAffinitySatisfied(pod, node, all, placed) &&
		volumesReachable(pod, node, claims, pvs)
}
...
func preempt(pod *corev1.Pod, all []*corev1.Node, placed []*corev1.Pod, claims corelisters.PersistentVolumeClaimLister, pvs corelisters.PersistentVolumeLister) (string, []*corev1.Pod) {
	var bestNode string
	var best []*corev1.Pod
	for _, n := range all {
		if !feasible(pod, n, all, placed, claims, pvs) {
			continue
		}
...
func run() error {
	...
	pick := func(pod *corev1.Pod) string {
		...
		placed = others(placed, pod)
		var candidates []*corev1.Node
		for _, n := range all {
			if feasible(pod, n, all, placed, claimLister, pvLister) && fits(pod, n, placed) {
	...
	schedule := func(pod *corev1.Pod, since int) {
		...
		if target == "" {
			...
			if all, err := nodeLister.List(labels.Everything()); err == nil {
				if placed, err := podLister.List(labels.Everything()); err == nil {
					placed = others(placed, pod)
					on, victims := "", []*corev1.Pod(nil)
					// Room already being made is room enough: victims take a
					// moment to go, and a pod that asks again while they do
					// empties a second node for nothing.
					if !evictionUnderway(pod, placed) {
						on, victims = preempt(pod, all, placed, claimLister, pvLister)
					}
					if on != "" {
...
func fits(pod *corev1.Pod, node *corev1.Node, placed []*corev1.Pod) bool {
	...
	for _, p := range placed {
		if occupies(p) != node.Name || p.Status.Phase == corev1.PodSucceeded || p.Status.Phase == corev1.PodFailed {
			continue
		}
		for name, q := range requests(p) {
			sum := used[name]
